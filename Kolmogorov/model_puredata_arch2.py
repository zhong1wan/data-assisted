...
class lstm_series(object):
	...
	def compile_time_step(self, y_input, initial_state=None):
		...
		return ys1, ys1_o, Bs, final_states
	...
	def compile(self):
		...
		self.Yseries, self.dYseries = [], []
		ys, Ys, dYs, lstm_states = self.compile_time_step(self.inputs)
		...
		for _ in range(self.l):
			...
			ys, Ys, dYs, lstm_states = self.compile_time_step(ys, lstm_states)
			self.Yseries.append(Ys)
			self.dYseries.append(dYs)
...
def main():
	...
	hid_units_B = 38
	s, l = 100, 50
	sp = './logs/lstm_seq2seq_5/step50/'
	pretrain = './logs/lstm_seq2seq_5/step20/weights.best.hdf5'
	# pretrain = './logs/lstm_hybrid_21/weights.best.hdf5'
